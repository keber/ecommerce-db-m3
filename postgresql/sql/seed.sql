...
-- Limpiar en orden inverso respetando FK; resetear secuencias
TRUNCATE TABLE
    payments,
    inventory_movements,
    inventory,
...
--    Nota: las cadenas de texto usan comillas simples con '' para escapar apóstrofes
-- =============================================================================
...
-- Personajes (6 productos)
  ...
  (38, 'Polera ''Linus Torvalds''',                              1, 8, 13990,
       'El creador del kernel Linux y el flamewar de MINIX más famoso de la historia.',
...
-- =============================================================================
-- 6. addresses  (1 por cliente; is_default usa literal booleano)
-- =============================================================================
INSERT INTO addresses (id, customer_id, street, city, region, postal_code, country, is_default) VALUES
  (1,  1,  'Av. Providencia 1234, Dpto 5B',   'Santiago',    'Región Metropolitana',            '7500000', 'CL', TRUE),
  (2,  2,  'Calle Los Andes 456',              'Valparaíso',  'Región de Valparaíso',            '2340000', 'CL', TRUE),
  (3,  3,  'Pasaje Las Flores 789',            'Concepción',  'Región del Biobío',               '4030000', 'CL', TRUE),
  (4,  4,  'Av. Alemania 321, Casa 3',         'Temuco',      'Región de La Araucanía',          '4780000', 'CL', TRUE),
  (5,  5,  'Calle Serrano 654',                'La Serena',   'Región de Coquimbo',              '1700000', 'CL', TRUE),
  (6,  6,  'Av. Balmaceda 987',                'Antofagasta', 'Región de Antofagasta',           '1240000', 'CL', TRUE),
  (7,  7,  'Av. Colón 147, Dpto 12',           'Puerto Montt','Región de Los Lagos',             '5480000', 'CL', TRUE),
  (8,  8,  'Los Pinos 258',                    'Rancagua',    'Región del Libertador B. O''H.',  '2820000', 'CL', TRUE),
  (9,  9,  'Av. Independencia 369',            'Arica',       'Región de Arica y Parinacota',    '1000000', 'CL', TRUE),
  (10, 10, 'Calle Arturo Prat 741',            'Iquique',     'Región de Tarapacá',              '1100000', 'CL', TRUE);

-- =============================================================================
-- 7. inventory  (snapshot inicial)
--    variant_id sigue el orden de inserción en product_variants:
--    producto 1 → variant_ids 1-4, producto 2 → 5-8, ..., producto 49 → 193-196
-- =============================================================================

-- Stock base: 20 unidades disponibles para todas las variantes
INSERT INTO inventory (variant_id, qty_available, qty_reserved)
SELECT id, 20, 0 FROM product_variants;

-- Ajustar stock bajo para ejercitar la consulta de stock crítico
...
-- =============================================================================
-- 9. order_items
--    unit_price capturado en el momento de la orden
--    variant_id: variantes talla M (posición 2 de cada grupo de 4)
-- =============================================================================
-- Mapa variant_id (talla M) por producto:
--  prod  1= 2, prod  2= 6, prod  3=10, prod  4=14, prod  5=18, prod  6=22,
--  prod  7=26, prod  8=30, prod  9=34, prod 10=38, prod 11=42, prod 12=46,
--  prod 13=50, prod 14=54, prod 15=58, prod 16=62, prod 17=66, prod 18=70,
--  prod 19=74, prod 20=78, prod 21=82, prod 22=86, prod 23=90, prod 24=94,
--  prod 25=98, prod 26=102,prod 27=106,prod 28=110,prod 29=114,prod 30=118,
--  prod 31=122,prod 32=126,prod 33=130,prod 34=134,prod 35=138,prod 36=142,
--  prod 37=146,prod 38=150,prod 39=154,prod 40=158,prod 41=162,prod 42=166,
--  prod 43=170,prod 44=174,prod 45=178,prod 46=182,prod 47=186,prod 48=190,
--  prod 49=194

INSERT INTO order_items (order_id, variant_id, unit_price, qty, subtotal) VALUES
  -- Orden 1: "No Deploy on Fridays" + "I Broke Prod Again"
  (1,  30, 14990, 1, 14990),
  (1,  22, 13990, 1, 13990),
  -- Orden 2: "Breaking Prod"
  (2,  10, 13990, 1, 13990),
  -- Orden 3: "Turing Test" + "Enigma Blueprint" + "Enigma Machine"
  (3,  142, 15990, 1, 15990),
  (3,  34,  15990, 1, 15990),
  (3,  38,  15990, 1, 15990),
  -- Orden 4: "Have You Tried..." + "Internet Emergency"
  (4,  74, 13990, 1, 13990),
  (4,  86, 13990, 1, 13990),
  -- Orden 5: "No Deploy on Fridays" x2 + "DevOps Acronym"
  (5,  30, 14990, 2, 29980),
  (5,  18, 12990, 1, 12990),
  -- Orden 6: "Breaking Prod"
  (6,  10, 13990, 1, 13990),
  -- Orden 7: "10% Writing Code..." + "Stack Overflow Driven Dev"
  (7,  178, 13990, 1, 13990),
  (7,  182, 13990, 1, 13990),
  -- Orden 8: "Cloud Architect"
  (8,  6, 14990, 1, 14990),
  -- Orden 9: "Turing Test" + "Nikola Tesla" + "Ada Lovelace"
  (9,  142, 15990, 1, 15990),
  (9,  138, 14990, 1, 14990),
  (9,  130, 14990, 1, 14990),
  -- Orden 10: "The Internet" x2 + "RTFM"
  (10, 114, 14990, 2, 29980),
  (10, 66,  12990, 1, 12990),
  -- Orden 11: "No Deploy on Fridays" + "It Works In My Container"
  (11, 30, 14990, 1, 14990),
  (11, 26, 13990, 1, 13990),
  -- Orden 12: "Enigma Blueprint" + "Enigma Machine"
  (12, 34, 15990, 1, 15990),
  (12, 38, 14990, 1, 14990),
  -- Orden 13: "Moss and Roy" x2 + "The Internet" + "0118..." + "Turing Test"
  (13, 94,  14990, 2, 29980),
  (13, 114, 14990, 1, 14990),
  (13, 62,  14990, 1, 14990),
  (13, 142, 15990, 1, 15990),
  -- Orden 14: "10% Writing Code..." x2 + "Debugging"
  (14, 178, 13990, 2, 27980),
  (14, 162, 13990, 1, 13990),
  -- Orden 15: "Cloud Architect" + "I Can Explain It To You"
  (15, 6,  14990, 1, 14990),
  (15, 2,  13990, 1, 13990),
  -- Orden 16: 7 it-crowd products
  (16, 74,  13990, 1, 13990),
  (16, 78,  13990, 1, 13990),
  (16, 82,  14990, 1, 14990),
  (16, 86,  13990, 1, 13990),
  ...
  (16, 94,  14990, 1, 14990),
  (16, 98,  13990, 1, 13990),
  -- Orden 17: "No Deploy on Fridays" + "It Works In My Container"
  (17, 30, 14990, 1, 14990),
  (17, 26, 13990, 1, 13990),
  -- Orden 18: "Enigma Machine"
  (18, 38, 15990, 1, 15990),
  -- Orden 19: "Ada Lovelace" + "Nikola Tesla" + "Von Neumann"
  (19, 130, 14990, 1, 14990),
  (19, 138, 14990, 1, 14990),
  (19, 146, 14990, 1, 14990),
  -- Orden 20: "Breaking Prod"
  (20, 10, 13990, 1, 13990),
  -- Orden 21: "No Deploy on Fridays" + "It Works In My Container"
  (21, 30, 14990, 1, 14990),
  (21, 26, 13990, 1, 13990),
  -- Orden 22: "10% Writing Code..." + "QA: I Don't Break Things"
  (22, 178, 13990, 1, 13990),
  (22, 190, 13990, 1, 13990),
  -- Orden 23: "Turing Test" + "Enigma Blueprint" + "Enigma Machine"
  (23, 142, 15990, 1, 15990),
  (23, 34,  15990, 1, 15990),
  (23, 38,  15990, 1, 15990),
  -- Orden 24: "Have You Tried..." + "Internet Emergency"
  (24, 74, 13990, 1, 13990),
  (24, 86, 13990, 1, 13990),
  -- Orden 25: "No Deploy on Fridays" x2 + "DevOps Acronym"
  (25, 30, 14990, 2, 29980),
  (25, 18, 12990, 1, 12990),
  -- Orden 26: "Cloud Architect"
  (26, 6, 14990, 1, 14990),
  -- Orden 27: "Breaking Prod"
  (27, 10, 13990, 1, 13990),
  -- Orden 28: "No Deploy on Fridays" + "It Works In My Container"
  (28, 30, 14990, 1, 14990),
  (28, 26, 13990, 1, 13990),
  -- Orden 29 (cancelada): "Ada Lovelace" + "Nikola Tesla" + "Von Neumann"
  (29, 130, 14990, 1, 14990),
  (29, 138, 14990, 1, 14990),
  (29, 146, 14990, 1, 14990),
  -- Orden 30: "Testing in Production" + "QA: I Don't Break Things"
  (30, 194, 14990, 1, 14990),
  (30, 190, 13990, 1, 13990);
...
-- =============================================================================
-- Resetear sequences después de inserts con IDs explícitos
-- (TRUNCATE RESTART IDENTITY los pone en 1; las inserciones con IDs explícitos
--  no avanzan la secuencia automáticamente en PostgreSQL / SERIAL)
-- =============================================================================
SELECT setval(pg_get_serial_sequence('product_types', 'id'), MAX(id)) FROM product_types;
